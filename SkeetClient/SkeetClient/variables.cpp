...
constexpr uint32 SOCKET_BUFFER_SIZE = 1024; // change this throughout client code
constexpr uint16 c_max_clients = 2;
...
{
	char buffer[256];
	sprintf(buffer, sizeof(buffer), fmt, arg);
...
{
	float32 time_take_s = time_since(tick_start_time, timing_info->clock_frequency);

	while (time_take_s < c_seconds_per_tick)
	{
		if (timing_info->sleep_granularity_was_set)
		{
			DWORD time_to_wait_ms = (DWORD)((c_seconds_per_tick - time_take_s) * 1000);
			if (time_to_wait_ms > 0)
			{
				Sleep(time_to_wait_ms);
			}
		}
		time_take_s = time_since(tick_start_time, timing_info->clock_frequency);
	}
}